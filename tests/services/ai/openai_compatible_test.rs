...
use xechat::services::ai::providers::openai_compatible::{
...
fn test_resolve_auth_headers_with_valid_key() {
    let params = make_params("sk-custom-key", "my-model", vec![]);
    let headers = resolve_auth_headers(&params);

    assert!(headers.is_some());
    let headers = headers.unwrap();
    let auth = headers.get("Authorization").unwrap();
    assert_eq!(auth, "Bearer sk-custom-key");
...
fn test_resolve_auth_headers_with_empty_key() {
    ...
    let params = make_params("", "my-model", vec![]);
    let headers = resolve_auth_headers(&params);
